...
void mouse(double &mx, double &my)
{
	...
	float x0, y0;
	tview->computeDrawPosition(g_gl_height, c, r, tw, th, x0, y0);
	x0 += xi;
...
int main()
{
	...
	tw = (float)(g_gl_width / tileSetCols);
	th = (float)(g_gl_height / tileSetRows);
	tw2 = th;
	th2 = th / 2.0f;
	...
	// };
	float vertices[] = {
		tw / 2, 0.0f, 0.0f, tileH2,	 // top
		tw, th / 2, tileW2, 0.0f,	 // right
		0.0f, th / 2, tileW2, tileH, // left
		tw / 2, th, tileW, tileH2,	 // bottom
		tw, th / 2, tileW2, 0.0f,	 // right
		0.0f, th / 2, tileW2, tileH	 // left
	};
	...
	while (!glfwWindowShouldClose(g_window))
	{
		...
		for (int r = 0; r < tmap->getHeight(); r++)
		{
			for (int c = 0; c < tmap->getWidth(); c++)
			{
				...
				tview->computeDrawPosition(g_gl_height, c, r, tw, th, x, y);
				...
				glUniform1f(glGetUniformLocation(shader_programme, "ty"), y);
				...
				glUniformMatrix4fv(glGetUniformLocation(shader_programme, "projection"), 1, GL_FALSE, glm::value_ptr(proj));
